"""
    Download All Comments From allSuccessfulProjects
    Kickstarter Project
--------------------------------------------------------------------------------
change log:
...
-------------------------------------------------------------------------------
notes:
    Do not run this code; the API code has been removed; it was written in Py2.7; it is for archiving how to download comments.
--------------------------------------------------------------------------------
contributors:
    Kevin:
        name:       Kevin Williams
...
pathOut             = pathMain+"comments_main/"
...
def writeResults(pid,contents,timeStamp,it):
    # write results to file
    with open(pathOut + "Listing_" + pid + "_" + timeStamp + "project_comments_"+str(it)+".html", 'w') as f:
        f.write(contents)       
...
def downloadUpdatePages(i,df):
    ...
    try:
        ...
        writeResults(pid,content,timeStamp,it)
        cont = True
        while cont == True: 
            try:
                soup        = BeautifulSoup(content)
                older       = soup.find("a", { "class" : "btn btn--light-blue btn--block mt3 older_comments" })['href']
                nextLink    = "https://www.kickstarter.com" + older
                r           = requestSub(nextLink)
                source      = r.text
